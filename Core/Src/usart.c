...
/* USER CODE BEGIN 1 */
int _write(int file, char* ptr,
           int len) {  // overloading printf() for UART with DMA
    char buffer[128];
    uint32_t current_tick = HAL_GetTick();  // get current time
    uint32_t elapsed_time =
        current_tick - last_tick;  // get difference from last time
    ...
    int copy_len = (len < (sizeof(buffer) - offset - 1))
                       ? len
