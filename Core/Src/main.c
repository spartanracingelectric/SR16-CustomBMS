...
int main(void) {
    ...
    modPackInfo.soc = MAX_ACCUMULATOR_CAPACITY;
    ...
    while (1) {
        ...
        if (TimerPacket_FixedPulse(&cycleTimeCap)) {
            ...
            Safety_checkVoltageFaults(
                &modPackInfo, &safetyFaults, &safetyWarnings, &safetyStates,
                &high_volt_fault_lock, &low_volt_hysteresis,
                &low_volt_fault_lock, &cell_imbalance_hysteresis);
            ...
            > 50)) { Balance_start((uint16_t *)modPackInfo.cell_volt,
            NUM_DEVICES, modPackInfo.cell_volt_lowest); } else if (BALANCE) {
