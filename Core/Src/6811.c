...
void Wakeup_Idle(void) {
    ...
    for (int i = 0; i < NUM_DEVICES; i++) {
        ...
        HAL_SPI_Transmit(&hspi1, &hex_ff, 1,
                         100);  // Send byte 0xFF to wake LTC up
...
LTC_SPI_StatusTypeDef Read_Cell_Volt(uint16_t *read_voltages) {
    ...
    uint8_t read_voltages_reg[ARR_SIZE_REG];  // Increased in size to handle
                                              // multiple devices

    for (uint8_t i = 0;
         i < (NUM_CELL_SERIES_GROUP / LTC_SERIES_GROUPS_PER_RDCV); i++) {
        uint8_t cmd[4];
        uint16_t cmd_pec;
        ...
        for (uint8_t dev_idx = 0; dev_idx < NUM_DEVICES; dev_idx++) {
            ...
            memcpy(&read_voltages[dev_idx * NUM_CELL_SERIES_GROUP +
                                  i * LTC_SERIES_GROUPS_PER_RDCV],
                   data_ptr, REG_LEN - 2);
        }
    }
...
/**
 * 	write command to all pwm registers. This setup only allows to use
 * 4b'1111 (HIGH) or 4b'0000 (LOW).
 * @param total_ic		total count of ic (daisy chain)
 * @param pwm			A two dimensional array of the configuration
 * data that will be written
 */
void LTC6811_WRPWM(uint8_t total_ic, uint8_t pwm) {
    // NOTE currently chaging this method to only assign a specific PWM to all
    // registers
    ...
    for (uint8_t current_ic = total_ic; current_ic > 0;
         current_ic--)  // executes for each ltc6811 in daisy chain, this loops
                        // starts with
    {
                              ...
                              // register
        {
            ...
            wrpwm_buffer[cmd_index] =
                pwm;  // adding the pwm data to the array to be sent
            cmd_index = cmd_index + 1;
        }

        pwm_pec = (uint16_t)LTC_Pec15_Calc(
            BYTES_IN_REG, &pwm);  // calculating the PEC for each ICs
    ...
    Wakeup_Idle();  // This will guarantee that the ltc6811 isoSPI port is
...
void LTC6811_WRCFG(
    ...
    uint8_t config[][6]  // A two dimensional array of the configuration data
    ...
    for (uint8_t current_ic = total_ic; current_ic > 0; current_ic--) {
        ...
        for (uint8_t current_byte = 0; current_byte < BYTES_IN_REG;
            ...
            wrcfg_buffer[cmd_index] =
                config[current_ic - 1]
                      [current_byte];  // adding the config data to the array to
                                       // be sent
            cmd_index = cmd_index + 1;
        }

        cfg_pec = (uint16_t)LTC_Pec15_Calc(
            BYTES_IN_REG,
 ...
 * @param comm[6]	A two dimensional array of the comm data that will be
 * written
 */
void LTC_WRCOMM(uint8_t total_ic, uint8_t comm[6]) {
                        ...
                        // starts with
    {
        ...
        {
            // current_byte is the byte counter
            wrcomm_buffer[cmd_index] =
                comm[current_byte];  // adding the config data to the array to
                                     // be sent
            cmd_index = cmd_index + 1;
        }
        comm_pec = (uint16_t)LTC_Pec15_Calc(
            BYTES_IN_REG, &comm[0]);  // calculating the PEC for each ICs
                                      // configuration register data
...
LTC_SPI_StatusTypeDef Read_Cell_Temps(uint16_t *read_auxiliary) {
    ...
    LTC_SPI_StatusTypeDef hal_ret;
    const uint8_t ARR_SIZE_REG = NUM_DEVICES * REG_LEN;
    uint8_t read_auxiliary_reg[ARR_SIZE_REG];  // Increased in size to handle
                                               // multiple devices
    ...
    for (uint8_t i = 0;
        ...
        for (uint8_t dev_idx = 0; dev_idx < NUM_DEVICES; dev_idx++) {
            ...
            memcpy(&read_auxiliary[dev_idx * NUM_AUX_SERIES_GROUPS +
                                   i * LTC_SERIES_GROUPS_PER_RDAUX],
                   data_ptr, REG_LEN - 2);
        }
    }
...
void LTC_ADAX(uint8_t MD,  // ADC Mode
    ...
    /*
     Wakeup_Idle (); //This will guarantee that the ltc6811 isoSPI port is
     awake. This command can be removed. output_low(LTC6811_CS);
...
int32_t LTC_POLLADC() {
    ...
    uint8_t dummy_tx =
        0x00;  // we need to send this to give clock LTC6811 so it can send data
    uint8_t cmd[4];
    uint16_t cmd_pec;
    ...
    while (((HAL_GetTick() - start_time) < 210000)) {  // timeout at 210ms
        LTC_nCS_Low();
        HAL_SPI_Transmit(&hspi1, &dummy_tx, 1,
                         100);  // Send dummy byte and get adc status
 ...
 * @param 	len		Number of bytes that will be used to calculate a
 * PEC
