...
void Safety_checkVoltageFaults(Accumulator *batt, uint8_t *fault,
    ...
    for (int i = 1; i < NUM_CELLS; i++) {
        ...
        if (batt->cell_volt_highest >= CELL_HIGH_VOLT_WARNING &&
            batt->cell_volt_highest < CELL_HIGH_VOLT_FAULT) {
            *warnings |= WARNING_BIT_HIGH_VOLT;
        }
        // high cell volt fault
        if ((batt->cell_volt_highest >=
             CELL_HIGH_VOLT_FAULT) /*&& ((*high_volt_hysteresis) > 0)*/) {
            *high_volt_fault_lock = 1;
            *fault |= FAULT_BIT_HIGH_VOLT;
            HAL_GPIO_WritePin(MCU_SHUTDOWN_SIGNAL_GPIO_Port,
                              MCU_SHUTDOWN_SIGNAL_Pin, GPIO_PIN_SET);
            //			printf("high voltage fault signal on\n");
        }
        // reset high cell volt fault
        else if (batt->cell_volt_highest <
                     (CELL_HIGH_VOLT_FAULT - FAULT_LOCK_MARGIN_HIGH_VOLT) &&
                 *high_volt_fault_lock == 1) {
            *high_volt_fault_lock = 0;
            *warnings &= ~WARNING_BIT_HIGH_VOLT;
            *fault &= ~FAULT_BIT_HIGH_VOLT;
            HAL_GPIO_WritePin(MCU_SHUTDOWN_SIGNAL_GPIO_Port,
                              MCU_SHUTDOWN_SIGNAL_Pin, GPIO_PIN_RESET);
        }
        ...
        else if (batt->cell_volt_lowest >
                     (CELL_LOW_VOLT_FAULT + FAULT_LOCK_MARGIN_LOW_VOLT) &&
                 *low_volt_hysteresis == 1) {
        ...
        if (batt->cell_volt_lowest <= CELL_LOW_VOLT_FAULT) {
            *low_volt_hysteresis =
                1;  // use hysteresis and spend 2 cycle to fault
        } else if (batt->cell_volt_lowest > CELL_LOW_VOLT_FAULT) {
            *low_volt_hysteresis =
                0;  // use hysteresis and spend 2 cycle to fault
        }

        // cell volt imbalance warning
        //		if ((batt->cell_volt_highest - batt->cell_volt_lowest)
        //>= CELL_VOLT_IMBALANCE_WARNING) { 			*warnings |=
        // WARNING_BIT_IMBALANCE;
        //		}
        //		//cell volt imbalance fault
        //		if (((batt->cell_volt_highest - batt->cell_volt_lowest)
        //>= CELL_VOLT_IMBALANCE_FAULT) && (*cell_imbalance_hysteresis == 1)) {
        ...
        //		else if (((batt->cell_volt_highest -
        // batt->cell_volt_lowest) < FAULT_LOCK_MARGIN_IMBALANCE) &&
        //(*cell_imbalance_hysteresis == 1)){
        //*cell_imbalance_hysteresis = 0; 			*warnings &=
        //~WARNING_BIT_IMBALANCE; 			*fault &=
        //~FAULT_BIT_IMBALANCE;
        // HAL_GPIO_WritePin(MCU_SHUTDOWN_SIGNAL_GPIO_Port,
        // MCU_SHUTDOWN_SIGNAL_Pin, GPIO_PIN_RESET);
        //		}
        ...
        //		}
        //		else if ((batt->cell_volt_highest -
        // batt->cell_volt_lowest) < CELL_VOLT_IMBALANCE_FAULT) {
...
void Safety_checkTempFaults(Accumulator *batt, uint8_t *fault,
    ...
    for (int i = 0; i < NUM_THERM_TOTAL; i++) {
        ...
        if (batt->cell_temp_highest >= CELL_HIGH_TEMP_WARNING &&
            batt->cell_temp_highest < CELL_HIGH_TEMP_FAULT) {
        ...
        else if (batt->cell_temp_highest <
                     (CELL_HIGH_TEMP_FAULT - FAULT_LOCK_MARGIN_HIGH_TEMP) &&
                 *high_temp_hysteresis == 1) {
            *warnings &= ~WARNING_BIT_HIGH_TEMP;
            *fault &= ~FAULT_BIT_HIGH_TEMP;
            HAL_GPIO_WritePin(MCU_SHUTDOWN_SIGNAL_GPIO_Port,
...
//         uint16_t volt_sum = 0;
//
//         for (int j = i; j < i + NUM_CELL_SERIES_GROUP && j < NUM_CELLS;
//         j++)
//         {
...
//         uint16_t temp_sum = 0;
//
//         for (int j = i; j < i + NUM_THERM_PER_MOD && j < NUM_THERM_TOTAL;
//         j++) {
